...
public class Slot : MonoBehaviour
{
    ...
    private GameObject itemPrefab;//需要存储的物品预设
    private Image image;
    private Color c;//original color
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            image.color = green;
            collision.gameObject.GetComponent<ItemUI>().inSlot++;
            collision.gameObject.GetComponent<ItemUI>().slotPos = collision.gameObject.GetComponent<ItemUI>().slotPos + transform.position;
            //占据这个格子
        }
    }
